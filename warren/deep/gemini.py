...
For every conclusion that relies on evidence.claims, add a citation entry. Use only claim IDs present in the packet. item_index is zero-based; use 0 for thesis. Do not cite a claim merely because it is topically related. Structured provider metric claims may support the exact financial observation they contain; deterministic category scores are never evidence.
